...
class CKT_CIKM(nn.Module):
    ...
    def forward(self, x):
        ...
        past_counts: same shape as x
        repeat_gaps: same shape as x
        ...
        # TODO 对x进行embedding
        embed_output = self.embedding(x)  # [batch, max_seq_len, 205] embed_dim=205
        ...
        cnn_3d_input = torch.cat([x for x in cnn_3d_input], 0) # cnn_3d_input: [batch_size, max_seq_len, k_frames, H, H]
        #########
...
#     model = CKT(k_frames, input_dim, H, embed_dim, hidden_dim, num_layers, output_dim, batch_size, device)

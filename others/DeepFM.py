...
class DeepFM(nn.Module):
    ...
    def forward(self, Xi, Xv):
        ...
        Inputs:
        - Xi: A tensor of input's index, shape of (N, field_size, 1) (batch_size,field_size,embedding_size)
        ...
        for i, emb in enumerate(self.fm_second_order_embeddings):
            ...
            em = emb(t)
            print(em.shape)
            print(torch.sum(em, 1).shape)
            print("=" * 50)
            print("Xv shape is: ", Xv[:, i].shape)
            # print((torch.sum(emb(Xi[:, i, :]), 1).t() * Xv[:, i]).t())
            print((torch.sum(emb(Xi[:, i, :]), 1).shape))


        fm_first_order_emb_arr = [(torch.sum(emb(Xi[:, i, :]), 1).t() * Xv[:, i]).t() for i, emb in
                                  enumerate(self.fm_first_order_embeddings)]
        print(fm_first_order_emb_arr[0].shape)
...
if __name__ == '__main__':
    ...
    embedding_size = 115
    use_cuda = False
    model = DeepFM(feature_sizes=feature_sizes, embedding_size=embedding_size, use_cuda=use_cuda)

    input = torch.randint(1, 220, (200, 1, 1))
    Xv = torch.ones((200, 1, 1))
    # print(input)
    model(input, Xv)
